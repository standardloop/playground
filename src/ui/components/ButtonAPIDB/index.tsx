...
class ButtonAPIDB extends React.Component<MyProps, MyState> {
  ...
  getNumber = () => {
    axios.get(`${process.env.API_PROTOCOOL}://${process.env.API_URL}:${process.env.API_PORT}/api/v1/randDB/`, {
    headers: {
  ...
  render() {
    ...
    return (
      <div className="Button">
        <button onClick={this.getNumber} className="buttonAPI">RandomNumberAPIFromDB</button>
        <h1>{ randomNumber }</h1>
      </div>
    );
  };
